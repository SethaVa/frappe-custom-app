...
export const getProductCartQuantity = (cartItems, product, color, size) => {
  ...
  if (cartItems.length >= 1 && productInCart) {
    if (product.variation) {
      ...
      return cartItems.find((single) => product?.name === single?.name)
        .quantity;
    }
  } else {
    return 0;
...
export const cartItemStock = (item, color, size) => {
  if (item.valuation_rate) {
    ...
    if (item.variation) {
      const existingItem = item?.variation.filter(
        (single) => single.color === color
      )[0];
      return existingItem
        ? existingItem.size.filter((single) => single.name === size)[0]
            .valuation_rate
        : 0;
    } else {
      return 0;
